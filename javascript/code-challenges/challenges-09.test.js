...
const maxInArray = (arr) => {
  let maxArray =arr.reduce( (accumulator, currentValue) => {
...
Write a function named checkValues that takes in an object and 
a value and returns true if the value is in the object.
...
const updateNumbers = (obj) => {
  ...
  let joinedArray = keysArray.map((key, idx) => {
    let string = `${key}: ${valuesArray[idx]}`;
    return string;
  });
  return joinedArray;
};
...
Write a function named hasChildrenValues that uses Object.
values to determine if any given character in the data set has children.
...
Write a function named hasChildrenEntries that is similar to your
 hasChildrenValues function from challenge 4, but uses the data's entries instead of its values.
