...
const addValues = (arr) => {
  let newArray=arr.reduce((previousValue, currentValue) => previousValue + currentValue,
  0);
  return newArray;
};
...
given an array of objects as input, uses reduce to
 find the total amount purchased. Each object contains the keys `item` and `purchasePrice` like the example.
...
const addPurchases = (arr) => {
  let totalAmount= arr.reduce((item,purchasePrice)=> { return item+= purchasePrice.purchasePrice}, 0);
  return totalAmount;
};
...
given an array as input, uses reduce to count the number of elements in the array.

Note: You may not use the array's built-in length property.
------------------------------------------------------------------------------------------------ */

const countNumberOfElements = (arr) => {
  let countArray = arr.reduce((previousValue, currentValue)=> {
...
Write a function named returnNames that, given the Star Wars data, below, uses reduce to return 
...
const returnNames = (arr) => {
  ...
  arr.reduce((previousValue, currentValue)=> {
    newArray.push(currentValue.name)
...
const reversedString = (str) => {
  ...
  let newString = newArray.reduce((val1,val2) => {
    val1 = val2 + val1;
    return val1;
  }, '');
  return newString
