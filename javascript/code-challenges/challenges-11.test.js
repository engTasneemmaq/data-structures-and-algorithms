...
function transformToLis(obj){
  let result = [];
  Object.entries(obj).forEach(([key, value]) => result.push(`<li>${key}: ${value}</li>`));
  return result;
};
...
let findMaleAndFemale = (data) => {
  let answer = [];
  data.map(obj => {
    return obj.gender === 'male' || obj.gender === 'female' ? answer.push(obj.name) : 0;
  });
...
from Challenge 6, uses any combination of filter, map and reduce to
 return the name of the character who is the shortest in height.
------------------------------------------------------------------------------------------------ */

let findShortest = (data) => {
  let currentLow = '200';
  let result = data.map( (character) => {
    character.height > currentLow ? currentLow = character.name : '';
  });
  return currentLow;
};
