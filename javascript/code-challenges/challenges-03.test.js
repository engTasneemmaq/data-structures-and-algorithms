...
Write a function called addTwo that takes in an array and adds two to 
every value using a for loop. Place the new value in a new array. Return the new array.
------------------------------------------------------------------------------------------------ */
...
const containsAnd = (arr) => {
  let onlyStringArray = arr.filter((string) => {
    return typeof(string) === 'string' && string.includes('and')});
...
const oddValues = (arr) => {
  let oddsArray = arr.filter(n => n%2)
  return oddsArray;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  let secondArray = arr.filter((second) => {
    return !(forbiddenValues.includes(second));
  });
  return secondArray;
};
